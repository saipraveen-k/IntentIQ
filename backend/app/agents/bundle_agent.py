...
class BundleAgent:
    ...
    async def get_or_create_bundles(
        ...
        if bundle_repo:
            ...
            if existing:
                ...
                return {
                    ...
                    "complete_the_look": bundled_prods[:half],
                    "frequently_bought_together": bundled_prods[half:],
                    "bundle_discount_pct": existing.discount_pct,
        ...
        for prod in other_candidates:
            if prod.id != base_product.id and prod.category != base_product.category:
        ...
        for prod in cat_candidates:
            if prod.id != base_product.id and len(frequently_bought) < 2:
                frequently_bought.append(prod)

        orig_total = base_product.price + sum(p.price for p in complete_look)
