...
def resolve_image(name, dept, aisle=''):
  n = name.lower() if name else ''
  d = dept.lower().strip() if dept else ''
  a = aisle.lower().strip() if aisle else ''
...
def generate():
  ...
  for row in top_df.itertuples():
    ...
    img_url = resolve_image(row.product_name, row.department, row.aisle)
